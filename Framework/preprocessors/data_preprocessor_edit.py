...
class DataPreprocessor:
    def __init__(self):
        ...
        self.split_train_to_train_and_valid = False
    ...
    @staticmethod
    def get_full_paths(files, path):
        return [os.path.join(x, path) for x in files]
    ...
    def preprocess_files(self,
        ...
        for single_file_full_path in all_files_full_paths:
            loaded_file = np.load(single_file_full_path)
    ...
    def preprocess_folder(self,
                          ...
                          split_train_to_train_and_valid: bool = False,
                          split_valid_train_ratio: float = .2) -> None:

        if data_type == 'Train' and split_train_to_train_and_valid:
            all_files_numpy = np.asarray(os.listdir(source_folder_path))
            np.random.shuffle(all_files_numpy)
            all_files_train = all_files_numpy[split_valid_train_ratio:]
            all_files_valid = all_files_numpy[:split_valid_train_ratio]

            all_files_train = self.get_full_paths(all_files_train, source_folder_path)
            all_files_valid = self.get_full_paths(all_files_valid, source_folder_path)


        else:
            all_files = os.listdir(source_folder_path)
            all_files = self.get_full_paths(all_files, source_folder_path)


    def preprocess_data_type(self,
                             data_type,
                             list_path,
                             preprocessing_type,
                             mix_bool,
                             ...
                             split_train_to_train_and_valid,
                             ) -> None:
    ...
    def preprocess_data(self,
                        ...
                        split_train_to_train_and_valid: bool = False) -> dict:
        ...
        for data_type, list_path in data_paths.items():
            ...
            self.preprocess_data_type(data_type,
                                      list_path,
                                      preprocessing_type,
                                      mix_bool,
                                      rewrite_data,
