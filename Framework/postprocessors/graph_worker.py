...
def get_distribution_plot(valid_predictions, test_predictions):
    ...
    fig, ax = plt.subplots(1,no_datasets_test+1, figsize=(5*no_datasets_test, 5))
    ax[0].bar(class_0_parameters[2][:-1], class_0_parameters[1], width=class_0_parameters[0], alpha=0.5, color='r',
              label='Normalized Histogram valid class 0')
    ax[0].bar(class_1_parameters[2][:-1], class_1_parameters[1], width=class_1_parameters[0], alpha=0.5, color='b',
              label='Normalized Histogram valid class 1')
    ax[0].set_title('Validation Error distribution')
    ...
    for id_dataset, dataset_score in enumerate(test_predictions):
        ...
        ax[id_dataset+1].bar(class_0_parameters[2][:-1], class_0_parameters[1], width=class_0_parameters[0], alpha=0.5, color='r',
              label='Normalized Histogram valid class 0')
        ax[id_dataset+1].bar(class_1_parameters[2][:-1], class_1_parameters[1], width=class_1_parameters[0], alpha=0.5, color='b',
              label='Normalized Histogram valid class 1')
        ax[id_dataset+1].set_ylim([0, 0.5])
        ax[id_dataset+1].legend()
        ax[id_dataset+1].grid(True)
        ax[id_dataset+1].set_xlabel('Anomaly Score')
        ax[id_dataset+1].set_ylabel('Density')
        ax[id_dataset+1].set_title(f'Test_dataset {id_dataset}, error distributions')
